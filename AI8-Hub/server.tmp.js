...
app.get("/admin/api/channels", requireAdminAuth, (req, res) => {
    res.json({
        data: getConfig().customChannels || [],
        ok: true
    });
});

app.put("/admin/api/channels", requireAdminAuth, (req, res) => {
...
app.get("/admin/api/models", requireAdminAuth, asyncHandler(async (req, res) => {
    ...
    const models = await fetchAggregatedModels(client, config, forceRefresh, logger);

    res.json({
        count: models.length,
        data: buildAdminModelsList(models),
        default_model: toDisplayModelId(config.ai8DefaultModel),
        object: "list",
...
app.get("/v1/models", asyncHandler(async (req, res) => {
    const config = getConfig();
    const client = getClient();
    const models = await fetchAggregatedModels(client, config, false, logger);
            ...
            for (const block of lines) {
                if (block.startsWith("data: ")) {
                    ...
                    try {
                        ...
                        const converted = openAiToAnthropicChunk(parsed);
                        if (isInitial) {
                            anthropicStream += `event: message_start\ndata: {"type":"message_start","message":{"id":"msg_${Date.now()}","type":"message","role":"assistant","model":"${parsed.model}"}}\n\n`;
                            isInitial = false;
                        }
                        if (converted) {
...
app.post("/v1/chat/completions", asyncHandler(async (req, res) => {
    ...
    const resolved = resolveTargetChannel(requestModel, config);
    ...
    console.log('ABOUT TO RESPOND JSON'); res.json(buildChatCompletion({
