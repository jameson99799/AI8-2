...
app.get("/admin/api/models", requireAdminAuth, asyncHandler(async (req, res) => {
    ...
    const models = await fetchAggregatedModels(client, config, forceRefresh, logger, true);
...
app.get("/v1/models", asyncHandler(async (req, res) => {
    const config = getConfig();
    const client = getClient();
    const models = await fetchAggregatedModels(client, config, false, logger, false);
